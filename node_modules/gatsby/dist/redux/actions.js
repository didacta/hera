...
const _require4 = require(`../schema/node-tracking`),
...
actions.createPage = (page, plugin, actionOptions) => {
  ...
  if (_.isObject(page.context)) {
    const reservedFields = [`path`, `matchPath`, `component`, `componentChunkName`, `pluginCreator___NODE`, `pluginCreatorName`];
  ...
  if (!internalPage.component.includes(`/.cache/`) && process.env.NODE_ENV === `production` && !fileOkCache[internalPage.component]) {
    ...
    if (!notEmpty || !includesDefaultExport) {
      ...
      if (!notEmpty) {
        ...
        console.log(``); // TODO actually do die during builds.
        // process.exit(1)
      }

      if (!includesDefaultExport) {
        console.log(``);
        console.log(`[${fileName}] The page component must export a React component for it to be valid`);
        console.log(``);
...
actions.deleteNode = (options, plugin, ...args) => {
  ...
  if (typeof options === `string`) {
    console.warn(`Calling "deleteNode" with a nodeId is deprecated. Please pass an object containing a full node instead: deleteNode({ node })`);
  ...
  let deleteDescendantsActions; // It's possible the file node was never created as sometimes tools will
  ...
  if (node) {
    ...
    if (descendantNodes.length > 0) {
      deleteDescendantsActions = descendantNodes.map(n => actions.deleteNode({
        node: getNode(n)
      }, plugin));
    }
  }
...
actions.deleteNodes = (nodes, plugin) => {
  console.log(`The "deleteNodes" action is now deprecated and will be removed in Gatsby v3. Please use "deleteNode" instead.`);
...
actions.createNode = (node, plugin, actionOptions = {}) => {
  ...
  if (node.internal.owner) {
    ...
    console.log(chalk.bold.red(`The node internal.owner field is set automatically by Gatsby and not by plugins`));
    process.exit(1);
  ...
  if (oldNode && !hasNodeChanged(node.id, node.internal.contentDigest)) {
    ...
    if (oldNode) {
      ...
      if (descendantNodes.length > 0) {
        deleteAction = descendantNodes.map(n => actions.deleteNode({
          node: getNode(n)
        }));
      }
    }
...
actions.createRedirect = (_ref) => {
  let fromPath = _ref.fromPath,
      ...
      toPath = _ref.toPath,
      rest = (0, _objectWithoutProperties2.default)(_ref, ["fromPath", "isPermanent", "redirectInBrowser", "toPath"]);
  let pathPrefix = ``;
